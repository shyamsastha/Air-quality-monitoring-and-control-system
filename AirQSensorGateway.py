    ...
    def run(self):
        while True:
            if self.enableAdaptor:
                ...
                displaymessage = "Temperature: " + str(self.sensorData.getTemperature()) + " °C "\
                "Pressure: " + str(self.sensorData.getPressure()) + " mbar "\
                "Humidity: " + str(self.sensorData.getHumidity()) + " % "
                '''
                ...
                if (self.sensorData.getTemperature()  >= 36):
                    ...
                    sense.show_message("Air Conditioner could be broken... Stats: " + \
                        displaymessage, scroll_speed=0.075)
                if (self.sensorData.getHumidity()  <= 34):
                    print('\n Triggering alert...')
                    self.connector.publishMessage('Humidifier needs a new filter!!'
                    'Or maybe stop wasting it needlessly :/\n', msg)
                    sense.show_message("Humidifier Needs a New filter... Stats: " + \
                        displaymessage, scroll_speed=0.075)
            sleep(15)
